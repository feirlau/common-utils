...
package com.fl.utils
{
    ...
    public class StringUtil
    {
        ...
        private static const LINE_BREAKS:RegExp = new RegExp("[\r\n]+", "img");
        public static function joinLines(value:String):String {
            LINE_BREAKS.lastIndex = 0;
            return value ? value.replace(LINE_BREAKS, "") : value;
        }
        
        public static function toFixed(value:Number, p:uint = 2, trimZero:Boolean = true):String {
            var tmpS:String = value.toFixed(p);
            if(trimZero && p > 0) {
                for(var i:int = tmpS.length - 1; i >= 0; i--) {

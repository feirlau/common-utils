...
import imp, os, xlrd, codecs, base64, zipfile, fnmatch, distutils.file_util
...
def getDataValue(cellValue, cellType):
    ...
    if(cellType == "int"):
        ...
        tmpValue = long(round(float(cellValue)))
...
def generateConfData(confFile):
    try:
        ...
        for i in range(1, dataSheet.nrows):
            ...
            for attrItem in attrList:
                ...
                try:
					...
					print 'value error:: column=' + colName + ', row=' + str(i) + ', type=' + colType
                except:
        ...
        print "Parse conf failed: %s"%confFile
        type_, value_, traceback_ = sys.exc_info()
        msg = traceback.format_exception(type_, value_, traceback_)
        print msg

default_includes = ["*.txt", "*.xml"]
def isInclude(f):
    result = False
    for s in default_includes:
...
def packConfData():
    try:
        dataFiles = os.listdir(default_output)
        dataFiles = [f for f in dataFiles if isInclude(f)]
        zipF = zipfile.ZipFile(os.path.join(default_output, "GameData.zip"), "w", zipfile.ZIP_DEFLATED)
        for dataFile in dataFiles:
            zipF.write(os.path.join(default_output, dataFile), dataFile)
        zipF.close()
        distutils.file_util.copy_file(os.path.join(default_output, "GameData.zip"), os.path.join(default_output, "GameData.swf"))
    except:
        type_, value_, traceback_ = sys.exc_info()
        print traceback.format_exception(type_, value_, traceback_)
def start():
